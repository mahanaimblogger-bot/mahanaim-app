...
const supabase = createClient(
  ...
  process.env.SUPABASE_SERVICE_ROLE_KEY  // usa service_role para evitar permisos
);
...
export async function POST(request) {
  ...
  // 1. Obtener el chapter_id real (de la tabla chapters)
  ...
  // 2. Generar biografía con IA (OpenRouter)
  const prompt = `
    Eres un experto en la Biblia. Genera una ficha de personaje para "${name}" en español.
    Devuelve solo un objeto JSON con:
    - biography: texto de 3-5 párrafos (biografía, contexto, eventos clave).
    - image_prompt: breve descripción para generar una imagen (opcional).
  ...
  const { biography, image_prompt } = JSON.parse(content);

  // 3. Crear slug único (ej: "abraham" , "moises")
  const slug = name
    .toLowerCase()
    .normalize('NFD').replace(/[\u0300-\u036f]/g, '')
    .replace(/[^a-z0-9]+/g, '-');

  // 4. Insertar en tabla persons
