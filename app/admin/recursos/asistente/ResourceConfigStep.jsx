...
export default function ResourceConfigStep({
  ctx,
  tipo,
  tipoInfo,
  onBack,
 ...
 const formatQuizInteractivo = (json) => {
  ...
  const formatContenidoHtml = (json) => {
    ...
    switch (json.tipo) {
      ...
      case "citas_teologos":
      case "citas_libros":
        return formatCitasHtml(json, tipoInfo?.label || "Citas");
             case "conexion_at":
  ...
  return (
          <div>
            ...
            <div style={{ marginTop: 10 }}>
              <button onClick={() => { onBack(); setGenerado(null); setErrorGen(null); }} style={{ background: "transparent", color: C.muted, border: `1px solid ${C.border}`, borderRadius: 7, padding: "8px 16px", cursor: "pointer", fontSize: 13 }}>
                ← Cambiar tipo
              </button>
            </div>
          </div>
