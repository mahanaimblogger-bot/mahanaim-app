...
export default function EditarCapituloPage() {
  ...
  const cargarDatos = async () => {
    ...
    // Cargar lista de libros para el select
    ...
    if (error) {
      ...
      setNumero(cap.numero || 1);
      setResumen(cap.resumen || '');
    }
    setLoading(false);
  };
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#fdfbf7]">
        <p className="text-[#8d6e63]">Cargando capítulo...</p>
      </div>
    );
  }
  if (error) {
      ...
      <main className="flex-1 p-8">
        <div className="max-w-3xl mx-auto bg-white border border-[#d4c4a8] rounded-xl shadow-sm p-8">
          <div className="flex items-center gap-4 mb-6">
            ...
            <h2 className="text-2xl font-bold text-[#1a5276]">Editar Capítulo</h2>
          </div>
